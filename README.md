...
- PyQt6 arayüzünde, header altında konumlanan reklam alanı `api/config.php` içindeki `ad_banner_html` ile veya `AD_BANNER_HTML` ortam değişkeniyle özelleştirilebilir; `ad_banner_enabled` değeriyle tamamen kapatılabilir (varsayılan 1). Banner, JavaScript destekli olarak QtWebEngine ile render edilir; Playwright sayfalarına artık enjekte edilmez.
- GUI’nin reklam alanı ve HTML/JS içeriğini sorunsuz gösterebilmesi için `PyQt6-WebEngine` bağımlılığı eklidir; `install.bat` otomatik kurar.
- İmleç stili/renkleri `cursor_style`, `cursor_primary`, `cursor_secondary` ve `cursor_styles` sözlüğü üzerinden ayarlanır; SVG overlay olarak enjekte edilir, yerel imleç gizlenir.
