...
- Playwright sayfalarına üstte kapatılabilir bir reklam bandı eklenir; HTML kodu `api/config.php` içindeki `ad_banner_html` ile veya `AD_BANNER_HTML` ortam değişkeniyle özelleştirilebilir. `ad_banner_enabled` ile banner tamamen kapatılabilir (varsayılan 1).
