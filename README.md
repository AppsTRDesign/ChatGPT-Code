...
A PHP 8 file upload and storage platform built for AlmaLinux deployments. The system provides SEO-friendly file sharing links, modern Bootstrap 5 UI enhanced with Dropzone drag & drop uploads, and AJAX-driven admin/client dashboards.
...
1. **Depoyu Kopyala:**
   ...
   cd fileupload
   ```
2. **PHP Bağımlılıkları:** Proje şu Composer paketlerine dayanır: `piwik/device-detector`, `geoip2/geoip2`, `phpmailer/phpmailer`, `iyzico/iyzipay-php`, `stripe/stripe-php`. Yüklemek için `composer install` komutunu çalıştırın.
