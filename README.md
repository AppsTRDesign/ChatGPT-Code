...
2. **Anahtar Üretin:** Lisansı dağıtan kişi hazır planları veya özel süreleri kullanabilir:
   ...
   ```
   > `generate_license.bat` sanal ortamı (`venv`) kullanır; önce `install.bat` çalıştırılmış olmalıdır.

   Windows dışındaki ortamlarda doğrudan `python licence/generate_license.py ...` veya etkin sanal ortamla `py licence/generate_license.py ...` komutunu kullanabilirsiniz.
