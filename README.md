...
- PyQt6 arayüzünde, header altında konumlanan reklam alanı `api/config.php` içindeki `ad_banner_html` ile veya `AD_BANNER_HTML` ortam değişkeniyle özelleştirilebilir; `ad_banner_enabled` değeriyle tamamen kapatılabilir (varsayılan 1). Playwright sayfalarına artık banner enjekte edilmez.
