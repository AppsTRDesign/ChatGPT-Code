...
- API ve bot sekmelerinde "Veri Alanları" kutucukları ile isim, adres, telefon, kategori, çalışma saatleri, puan, toplam değerlendirme, web sitesi ve işletme görseli alanlarını isteğe bağlı olarak açıp kapatma (varsayılan olarak yalnızca temel kimlik bilgileri seçilidir)
...
### Sosyal Giriş (Google/Apple/Facebook/Twitter)

Mobil uygulama için sosyal giriş API'si `api-app/social_login.php` üzerinden sağlanır. İstek gövdesi JSON olarak aşağıdaki alanları alır:

- `provider`: `google`, `apple`, `facebook`, `twitter`
- `email`: sağlayıcıdan gelen e‑posta (zorunlu)
- `name`: kullanıcı adı (opsiyonel)
- `avatar_url`: profil görseli (opsiyonel)
...
> Not: Üretim ortamında sağlayıcıdan aldığınız kimlik belirteçlerini (ID token / access token) doğrulamak için `api-app/social_login.php` içine doğrulama adımı ekleyin. Şu anki uç nokta, istemciden gelen e‑posta bilgisine göre kullanıcıyı oluşturur veya oturum açar.
...
- `backend/api` klasörü, AlmaLinux 8/Plesk üzerinde PHP 8 + PDO-MySQL ile çalışacak hafif bir API köprüsü içerir. `backend/api/schema.sql` dosyasını kullanarak MySQL veritabanınızı oluşturun (`places` tablosu ad/adres/koordinatlar/puan/yorum sayısı/telefon ve tipi/web sitesi/kategori/çalışma-yoğunluk saatleri/işletme görseli ve yorum JSON'unu tutar) ve `backend/api/config.php` içindeki `DB_DSN`/`DB_USER`/`DB_PASS` ve `API_TOKEN` alanlarını sunucunuza göre güncelleyin (env değişkenleri ile de ayarlanabilir). Varsayılan `API_TOKEN` değeri `maps-default-token` olarak gelir; üretimde mutlaka değiştirip aynı anahtarı uygulamadaki **API Anahtarı** alanına yazın (ya da her iki tarafa da `MAPS_API_TOKEN` ortam değişkenini ekleyin).
...
- API uç noktası `token` alanı ya da `X-API-Key` başlığı ile gelen anahtarı doğrular, her işletme satırını `places` tablosuna yazar ve dış erişime kapalı olacak şekilde arka uçta saklar. Anahtar uyuşmazsa 401 hatası alırsınız.
