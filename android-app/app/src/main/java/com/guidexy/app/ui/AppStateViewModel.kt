    ...
    private fun loadSearchPage(reset: Boolean) {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                val combined = current + response.data
                _searchResults.value = combined
                val total = response.total ?: combined.size
                _searchTotal.value = total
                _searchPage.value = response.page ?: searchPage
                _searchHasMore.value = if (response.total != null) {
                    combined.size < total
                } else {
                    response.data.size >= 20
                }
            }
