...
class AppStateViewModel(
    ...
    private val _favoritePlaces = MutableStateFlow<List<PlaceDto>>(emptyList())
    val favoritePlaces: StateFlow<List<PlaceDto>> = _favoritePlaces

    private val _recentReviews = MutableStateFlow<List<RecentReviewDto>>(emptyList())
    val recentReviews: StateFlow<List<RecentReviewDto>> = _recentReviews
    ...
    private fun loadHome(code: String) {
        viewModelScope.launch {
            _loading.value = true
            runCatching { repository.getLatest(code, PaginationConfig.homeLatestCount, 1) }
                .onSuccess { _latestPlaces.value = it.places }
            runCatching { repository.getPopular(code, PaginationConfig.homePopularCount, 1) }
                .onSuccess { _popularPlaces.value = it.places }
            runCatching { repository.getMostViewed(code, PaginationConfig.homeMostViewedCount, 1) }
                .onSuccess { _mostViewedPlaces.value = it.places }
            runCatching { repository.getFavorites(code, PaginationConfig.homeFavoritesCount, 1) }
                .onSuccess { _favoritePlaces.value = it.places }
            runCatching { repository.getRecentReviews(PaginationConfig.homeRecentReviewsCount, 1, code, "new") }
                .onSuccess { _recentReviews.value = it.reviews }
            _loading.value = false
        }
    }
    ...
    fun searchPlaces(
        ...
        lastSearchParams = SearchParams(query, category, city, country, sort, lat, lng)
        searchPageIndex = 1
    ...
    private fun loadSearchPage(reset: Boolean) {
        viewModelScope.launch {
            ...
            val params = lastSearchParams ?: run {
