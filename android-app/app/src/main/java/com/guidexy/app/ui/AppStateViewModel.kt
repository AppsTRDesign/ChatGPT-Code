...
class AppStateViewModel(
    ...
    private fun loadHome(code: String) {
        viewModelScope.launch {
            _loading.value = true
            runCatching { repository.getLatest(code, PaginationConfig.homeHighlightsCount) }
                .onSuccess { _latestPlaces.value = it.places }
            runCatching { repository.getPopular(code, PaginationConfig.homeHighlightsCount) }
    ...
    fun goToSearchPage(page: Int) {
        val total = _searchTotal.value
        val totalPages = if (total > 0) {
            ceil(total / PaginationConfig.resultsPerPage.toDouble()).toInt().coerceAtLeast(1)
        } else {
            1
        }
        val target = page.coerceIn(1, totalPages)
        if (target == searchPageIndex) return
        searchPageIndex = target
        _searchPage.value = searchPageIndex
        loadSearchPage(reset = true)
    }
