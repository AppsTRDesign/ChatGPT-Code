    ...
    androidx.compose.ui.viewinterop.AndroidView(
        ...
        update = { mapView ->
            ...
            places.forEach { place ->
                ...
                if (!imageUrl.isNullOrBlank() && markerBaseBitmap != null) {
                    scope.launch {
...
private class ColorRadiusMarkerClusterer(
    ...
    override fun buildClusterMarker(cluster: ArrayList<Marker>, mapView: MapView): Marker {
        setIcon(iconProvider(cluster.size))
...
private fun createPinThumbnailBitmap(pin: Bitmap, thumb: Bitmap): Bitmap {
    ...
    val size = (result.width * 0.5f).coerceAtLeast(16f)
    val left = (result.width - size) / 2f
    val top = (result.height * 0.15f).coerceAtLeast(2f)
