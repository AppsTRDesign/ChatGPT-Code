...
fun MapSection(
    ...
    // Performans için ikonları bir kez belleğe alıyoruz (Cache)
    val markerBaseBitmap = remember {
    ...
    androidx.compose.ui.viewinterop.AndroidView(
        ...
        factory = { ctx ->
            ...
            MapView(ctx).apply {
                ...
                // Donanım ivmelendirmeyi zorla
                setLayerType(android.view.View.LAYER_TYPE_HARDWARE, null)
            }
        },
        update = { mapView ->
            // 1. Veri kontrolü (Aynı veri setiyse tekrar çizme)
            val placeKey = places.hashCode().toString()
            if (mapView.tag == placeKey) return@AndroidView
            mapView.tag = placeKey

            // 2. Temizlik
            mapView.overlays.clear()

            // 3. Clusterer Ayarları
            val clusterer = RadiusMarkerClusterer(context).apply {
                setRadius(100)
                // Cluster ikonlarını cache'den alacak şekilde optimize edin
                setIcon(createClusterBitmap(context, 0xFF43A047.toInt()))
            }
            ...
            places.forEachIndexed { index, place ->
                ...
                val marker = Marker(mapView).apply {
                    ...
                    // Her seferinde drawable oluşturmak yerine hazır bitmap kullanın
                    icon = android.graphics.drawable.BitmapDrawable(context.resources, markerBaseBitmap)
                ...
                // 5. Pin Üzerine Resim Koyma İşlemini Sınırla
                // ANR'yi önlemek için sadece çok gerekliyse ve düşük sayıda yapın
                val imageUrl = place.business_image
                if (index < 15 && !imageUrl.isNullOrBlank() && markerBaseBitmap != null) {
                    scope.launch {
                        ...
                        if (thumb != null) {
                            ...
                            // Invalidate yerine sadece markerı yenilemeye çalışın
                        }
                    }
                }
            ...
            // 6. Kamera Odaklamayı Coroutine içine alarak UI thread'i rahatlatın
            if (places.isNotEmpty()) {
                val points = places.mapNotNull { p ->
                    val lt = p.latitude?.toDoubleOrNull() ?: return@mapNotNull null
                    val lg = p.longitude?.toDoubleOrNull() ?: return@mapNotNull null
                    GeoPoint(lt, lg)
                }
                if (points.isNotEmpty()) {
                    val box = BoundingBox.fromGeoPoints(points)
                    mapView.post { // Harita hazır olduğunda odakla
