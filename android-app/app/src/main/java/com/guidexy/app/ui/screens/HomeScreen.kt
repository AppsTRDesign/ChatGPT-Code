...
import com.guidexy.app.ui.components.HomeRecentReviewCard
    ...
    Scaffold(
        ...
        LazyVerticalGrid(
            ...
            if (recentReviews.isNotEmpty()) {
                item(span = { GridItemSpan(maxLineSpan) }) {
                    SectionHeader(
                        title = "Son Yapılan Yorumlar",
                        actionLabel = "Tümünü Gör",
                        onAction = { navController.navigate("recent_reviews") }
                    )
                }
                item(span = { GridItemSpan(maxLineSpan) }) {
                    LazyRow(
                        ...
                        items(
                            items = recentReviews.take(PaginationConfig.homeRecentReviewsCount),
                            key = { review -> "${review.source}_${review.review_ref}" }
                        ) { review ->
                            HomeRecentReviewCard(
                                review = review,
                                onClick = {
