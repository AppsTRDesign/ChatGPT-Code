...
fun PlaceListScreen(
    ...
    LaunchedEffect(selectedCountry?.code, pageState.value) {
        ...
        placesState.value = response.places
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Text("Sayfa ${pageState.value}", style = MaterialTheme.typography.labelMedium)
        LazyColumn(
            state = listState,
            ...
            if (loadingState.value) {
                item { Text("Yükleniyor...") }
            } else if (!hasMoreState.value) {
                item { Text("Tüm sonuçlar yüklendi.") }
            }
