    ...
    LaunchedEffect(selectedCountry?.code, pageState.value) {
        ...
        val responsePlaces = withContext(Dispatchers.IO) {
            if (listType == PlaceListType.Popular) {
                ApiClient.service.popular(code, limit).places
            } else {
                ApiClient.service.latest(code, limit).places
            }
        }
        val current = placesState.value
        val newItems = responsePlaces.drop(current.size)
        placesState.value = current + newItems
        hasMoreState.value = responsePlaces.size >= limit
