...
fun PlaceListScreen(
    ...
    val sortedPlaces = remember(placesState.value, selectedSort.value, lastLocation.value) {
        when (selectedSort.value.key) {
            "name_asc" -> placesState.value.sortedBy { it.name.lowercase() }
            "name_desc" -> placesState.value.sortedByDescending { it.name.lowercase() }
            "rating_desc" -> placesState.value.sortedByDescending { it.combined_rating ?: it.rating ?: 0.0 }
            "views" -> placesState.value.sortedByDescending { it.views ?: 0 }
            "distance" -> {
                val origin = lastLocation.value
                if (origin == null) {
                    placesState.value
                } else {
                    placesState.value.sortedBy { place ->
                        calculateDistanceMeters(origin, place.latitude, place.longitude)
                    }
                }
            }
            else -> placesState.value
        }
    }
