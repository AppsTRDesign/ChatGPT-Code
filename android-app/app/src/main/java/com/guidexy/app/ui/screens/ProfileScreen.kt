...
private fun ProfileLoggedOut(
    ...
    // 1. Google Konfigürasyonu
    val gso = remember {
        GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
            .requestIdToken("1063276280060-ufkg168easlt949km6r6bk7s3u6bg8af.apps.googleusercontent.com")
            .requestEmail()
            .build()
    }
    val googleSignInClient = remember { GoogleSignIn.getClient(context, gso) }

// 2. Google Login İşlemi (Direkt Kayıt/Giriş)
    val googleLauncher = rememberLauncherForActivityResult(
        ...
        try {
            ...
            if (idToken != null) {
                scope.launch {
                    try {
                        val response = ApiClient.service.socialLogin(
                            SocialAuthRequest(
                                provider = "google",
                                id_token = idToken,
                        ...
                        if (response.success!! && response.user != null) {
                            ...
                            Firebase.analytics.setUserId(response.user.id.toString())
                        ...
                        Toast.makeText(context, "Giriş yapılamadı: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        } catch (e: ApiException) {
            Toast.makeText(context, "Google Hatası: ${e.statusCode}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    LazyColumn(
        ...
        item {
            SocialLoginButtons(
                ...
                onProviderSelected = { provider -> /* Diğerleri için */ }
            )
        }
...
private fun SocialLoginButtons(
    ...
    Column(
        ...
        Text("Sosyal hesapla devam et", style = MaterialTheme.typography.labelLarge)
        ...
        Row(
            ...
            OutlinedButton(
                ...
                onClick = onGoogleClick // Google launcher'ını tetikler
            ) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    // İkon yeri (İstersen daha sonra buraya Icon() ekleyebilirsin)
                    Text("Google")
                }
            }
