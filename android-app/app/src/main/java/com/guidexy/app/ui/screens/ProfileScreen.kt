...
import com.guidexy.app.data.SocialAuthRequest
...
private fun ProfileLoggedOut(
    ...
    val socialDialogProvider = remember { mutableStateOf<String?>(null) }
    val socialName = remember { mutableStateOf("") }
    val socialEmail = remember { mutableStateOf("") }
    val socialLoading = remember { mutableStateOf(false) }
        ...
        item {
            SocialLoginButtons(
                onProviderSelected = { provider ->
                    socialDialogProvider.value = provider
                    socialName.value = ""
                    socialEmail.value = ""
                }
            )
    ...
    val provider = socialDialogProvider.value
    if (provider != null) {
        SocialLoginDialog(
            provider = provider,
            name = socialName.value,
            email = socialEmail.value,
            loading = socialLoading.value,
            onNameChange = { socialName.value = it },
            onEmailChange = { socialEmail.value = it },
            onDismiss = { if (!socialLoading.value) socialDialogProvider.value = null },
            onConfirm = { name, email ->
                scope.launch {
                    socialLoading.value = true
                    onStatusChange(null)
                    val response = runCatching {
                        ApiClient.service.socialLogin(
                            SocialAuthRequest(
                                provider = provider,
                                email = email,
                    ...
                    if (response?.success == true && response.user != null) {
                        UserSession.update(response.user)
                        onLoginSuccess(response.user)
                        socialDialogProvider.value = null
                        Toast.makeText(context, "Sosyal giriş başarılı.", Toast.LENGTH_SHORT).show()
                    } else {
                        onStatusChange("Sosyal giriş başarısız.")
                        Toast.makeText(context, "Sosyal giriş başarısız.", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        )
...
private fun SocialLoginButtons(
    ...
    Column(
        ...
        Row(
            ...
            OutlinedButton(
                modifier = Modifier.weight(1f),
                onClick = { onProviderSelected("twitter") }
            ) {
                Text("Twitter")
            }
        }
        Text(
            "Butonlar, sağlayıcı hesabınızdan gelen bilgileri kullanır.",
            style = MaterialTheme.typography.labelSmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant
        )
    }
...
private fun SocialLoginDialog(
    ...
    AlertDialog(
        ...
        title = { Text("$providerLabel ile devam et") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(
                    "Sağlayıcıdan gelen bilgileri kullanarak hesabınıza giriş yapın.",
...
private fun ProfileLoggedIn(
    ...
    val reviewApprovedPage = remember { mutableStateOf(1) }
    val reviewPendingPage = remember { mutableStateOf(1) }
    val reviewAnsweredPage = remember { mutableStateOf(1) }
    val reviewUnansweredPage = remember { mutableStateOf(1) }
    val reviewApprovedTotalPages = remember { mutableStateOf(1) }
    val reviewPendingTotalPages = remember { mutableStateOf(1) }
    val reviewAnsweredTotalPages = remember { mutableStateOf(1) }
    val reviewUnansweredTotalPages = remember { mutableStateOf(1) }
    val reviewApprovedTotal = remember { mutableStateOf(0) }
    val reviewPendingTotal = remember { mutableStateOf(0) }
    ...
    val favoritePlaces = remember { mutableStateOf<List<PlaceDto>>(emptyList()) }
    val favoritePlacesPage = remember { mutableStateOf(1) }
    val favoritePlacesTotalPages = remember { mutableStateOf(1) }
    val favoritePlacesTotal = remember { mutableStateOf(0) }
    val favoriteSortOptions = remember {
        listOf(
    ...
    val favoriteSort = remember { mutableStateOf(favoriteSortOptions.first()) }
    val favoriteSortExpanded = remember { mutableStateOf(false) }
    val favoriteLocation = remember { mutableStateOf<Pair<Double, Double>?>(null) }
    val pendingFavoriteDistance = remember { mutableStateOf(false) }
    ...
    val mainTabs = remember(isBusinessOwner) {
        if (isBusinessOwner) {
            listOf("Profil", "Profil Düzenle", "Yorumlarım", "Favorilerim", "İşletmelerim", "İşletme Yorumları")
        } else {
            listOf("Profil", "Profil Düzenle", "Yorumlarım", "Favorilerim")
        }
    }
    val favoriteTabIndex = if (isBusinessOwner) 3 else 3
    val ownerPlacesTabIndex = if (isBusinessOwner) 4 else -1
    val ownerReviewsTabIndex = if (isBusinessOwner) 5 else -1
    val context = androidx.compose.ui.platform.LocalContext.current
    val requestFavoriteLocationPermission = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { granted ->
        if (granted) {
            pendingFavoriteDistance.value = true
        }
    }
    val fetchFavoriteLocation = {
        val permission = Manifest.permission.ACCESS_FINE_LOCATION
        if (ContextCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
            requestFavoriteLocationPermission.launch(permission)
        } else {
            val fusedLocation = LocationServices.getFusedLocationProviderClient(context)
            fusedLocation.lastLocation.addOnSuccessListener { location ->
                if (location != null) {
                    favoriteLocation.value = location.latitude to location.longitude
                    favoritePlacesPage.value = 1
                }
            }
        }
    }
    ...
    LaunchedEffect(pendingFavoriteDistance.value) {
        if (pendingFavoriteDistance.value) {
            pendingFavoriteDistance.value = false
            fetchFavoriteLocation()
        }
    }

    LaunchedEffect(
        reviewTabIndex.value,
        reviewApprovedPage.value,
        reviewPendingPage.value,
        reviewAnsweredPage.value,
        reviewUnansweredPage.value,
        profile?.id
    ) {
        val userId = profile?.id ?: return@LaunchedEffect
        val response = when (reviewTabIndex.value) {
            0 -> runCatching {
                ApiClient.service.userReviews(userId, reviewApprovedPage.value, status = "approved")
            }.getOrNull()
            1 -> runCatching {
                ApiClient.service.userReviews(userId, reviewPendingPage.value, status = "pending")
            }.getOrNull()
            2 -> runCatching {
        ...
        if (response?.success == true) {
            ...
            when (reviewTabIndex.value) {
                0 -> {
                    reviewApprovedTotalPages.value = response.total_pages
                    reviewApprovedTotal.value = response.total
                }
                1 -> {
                    reviewPendingTotalPages.value = response.total_pages
                    reviewPendingTotal.value = response.total
                }
                2 -> {
    ...
    LaunchedEffect(profile?.id) {
        ...
        val approved = runCatching {
            ApiClient.service.userReviews(userId, 1, status = "approved")
        }.getOrNull()
        if (approved?.success == true) {
            reviewApprovedTotal.value = approved.total
            reviewApprovedTotalPages.value = approved.total_pages
        }
        val pending = runCatching {
            ApiClient.service.userReviews(userId, 1, status = "pending")
        }.getOrNull()
        if (pending?.success == true) {
            reviewPendingTotal.value = pending.total
            reviewPendingTotalPages.value = pending.total_pages
        }
        val answered = runCatching {
            ApiClient.service.userReviews(userId, 1, replyStatus = "answered")
        }.getOrNull()
        if (answered?.success == true) {
            reviewAnsweredTotal.value = answered.total
            reviewAnsweredTotalPages.value = answered.total_pages
        }
        val unanswered = runCatching {
            ApiClient.service.userReviews(userId, 1, replyStatus = "unanswered")
        }.getOrNull()
        if (unanswered?.success == true) {
            reviewUnansweredTotal.value = unanswered.total
            reviewUnansweredTotalPages.value = unanswered.total_pages
        }
    }

    LaunchedEffect(
        favoritePlacesPage.value,
        favoriteSort.value,
        favoriteLocation.value,
        profile?.id,
        tabIndex.value
    ) {
        if (tabIndex.value != favoriteTabIndex) return@LaunchedEffect
        val userId = profile?.id ?: return@LaunchedEffect
        val response = runCatching {
            ApiClient.service.placeFavorites(
                userId = userId,
                page = favoritePlacesPage.value,
                sort = favoriteSort.value.key,
                lat = favoriteLocation.value?.first,
                lng = favoriteLocation.value?.second
            )
        }.getOrNull()
        if (response?.success == true) {
            favoritePlaces.value = response.places
            favoritePlacesTotalPages.value = response.total_pages
            favoritePlacesTotal.value = response.total
        }
    }
    ...
    LaunchedEffect(
        ...
        val response = runCatching {
            ApiClient.service.placeReviews(
                placeId,
                ownerAnsweredPage.value,
                "new",
                status = "answered"
    ...
    Column(
        ...
        when (tabIndex.value) {
            ...
            2 -> {
                ...
                val activePage = when (reviewTabIndex.value) {
                    0 -> reviewApprovedPage.value
                    1 -> reviewPendingPage.value
                    2 -> reviewAnsweredPage.value
                    else -> reviewUnansweredPage.value
                }
                val activeTotalPages = when (reviewTabIndex.value) {
                    0 -> reviewApprovedTotalPages.value
                    1 -> reviewPendingTotalPages.value
                    2 -> reviewAnsweredTotalPages.value
                    else -> reviewUnansweredTotalPages.value
                }
                LazyColumn(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.spacedBy(12.dp)
                            ...
                            Column(
                                ...
                                Row(
                                    ...
                                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                        Text("Onaylanan", style = MaterialTheme.typography.labelSmall)
                                        Text(
                                            reviewApprovedTotal.value.toString(),
                                    ...
                                    Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                        Text("Bekleyen", style = MaterialTheme.typography.labelSmall)
                                        Text(
                                            reviewPendingTotal.value.toString(),
                                            style = MaterialTheme.typography.titleMedium,
                                            fontWeight = FontWeight.Bold,
                                            color = MaterialTheme.colorScheme.onSurfaceVariant
                                        )
                        ...
                        ScrollableTabRow(selectedTabIndex = reviewTabIndex.value, edgePadding = 0.dp) {
                            ...
                            Tab(
                                ...
                                onClick = {
                                    ...
                                    reviewPendingPage.value = 1
                                },
                                text = { Text("Bekleyen") }
                            )
                            Tab(
                                selected = reviewTabIndex.value == 2,
                                onClick = {
                                    reviewTabIndex.value = 2
                                    reviewAnsweredPage.value = 1
                                },
                                text = { Text("Yanıtlanan") }
                            )
                            Tab(
                                selected = reviewTabIndex.value == 3,
                                onClick = {
                        ...
                        Row(
                            ...
                            TextButton(
                                onClick = {
                                    when (reviewTabIndex.value) {
                                        0 -> reviewApprovedPage.value =
                                            (reviewApprovedPage.value - 1).coerceAtLeast(1)
                                        1 -> reviewPendingPage.value =
                                            (reviewPendingPage.value - 1).coerceAtLeast(1)
                                        2 -> reviewAnsweredPage.value =
                            ...
                            TextButton(
                                onClick = {
                                    when (reviewTabIndex.value) {
                                        0 -> reviewApprovedPage.value =
                                            (reviewApprovedPage.value + 1)
                                                .coerceAtMost(reviewApprovedTotalPages.value)
                                        1 -> reviewPendingPage.value =
                                            (reviewPendingPage.value + 1)
                                                .coerceAtMost(reviewPendingTotalPages.value)
                                        2 -> reviewAnsweredPage.value =
                                            (reviewAnsweredPage.value + 1)
                                                .coerceAtMost(reviewAnsweredTotalPages.value)
                                        else -> reviewUnansweredPage.value =
                                            (reviewUnansweredPage.value + 1)
                                                .coerceAtMost(reviewUnansweredTotalPages.value)
                                    }
                                },
            ...
            favoriteTabIndex -> {
                val totalFavorites = favoritePlacesTotal.value
                LazyColumn(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.spacedBy(12.dp)
                ) {
                    item {
                        Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                            Text(
                                "$totalFavorites sonuç bulundu",
                                style = MaterialTheme.typography.labelLarge
                            )
                            Spacer(Modifier.weight(1f))
                            Box {
                                TextButton(onClick = { favoriteSortExpanded.value = true }) {
                                    Icon(Icons.Default.Sort, null)
                                    Text(favoriteSort.value.label)
                                }
                                DropdownMenu(
                                    expanded = favoriteSortExpanded.value,
                                    onDismissRequest = { favoriteSortExpanded.value = false }
                                ) {
                                    favoriteSortOptions.forEach { option ->
                                        DropdownMenuItem(
                                            text = { Text(option.label) },
                                            onClick = {
                                                favoriteSort.value = option
                                                favoriteSortExpanded.value = false
                                                favoritePlacesPage.value = 1
                    ...
                    if (favoritePlaces.value.isEmpty()) {
                        item {
                            Text(
                                "Favori işletme bulunamadı.",
                                style = MaterialTheme.typography.bodyMedium,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                    } else {
                        items(favoritePlaces.value) { place ->
                    ...
                    item {
                        Row(
                            ...
                            TextButton(
                                onClick = {
                                    favoritePlacesPage.value = (favoritePlacesPage.value - 1).coerceAtLeast(1)
                                },
                                enabled = favoritePlacesPage.value > 1
                            ) {
                                Text("Önceki")
                            }
                            Text(
                                "${favoritePlacesPage.value}/${favoritePlacesTotalPages.value}",
                                style = MaterialTheme.typography.labelMedium
                            )
                            TextButton(
                                onClick = {
                                    favoritePlacesPage.value =
                                        (favoritePlacesPage.value + 1)
                                            .coerceAtMost(favoritePlacesTotalPages.value)
                                },
                                enabled = favoritePlacesPage.value < favoritePlacesTotalPages.value
                            ) {
                                Text("Sonraki")
                            }
                        }
