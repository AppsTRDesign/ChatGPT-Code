...
fun ProfileScreen() {
    ...
    LaunchedEffect(Unit) {
        val response = runCatching { ApiClient.service.profile() }.getOrNull()
                        ...
                        scope.launch {
                            ...
                            if (response?.success == true) {
                                ...
                                if (profile?.user != null) {
                                    ...
                                    onStatusChange("Profil bilgisi alınamadı.")
                                }
                            } else {
...
private fun ProfileLoggedIn(
    ...
    val password = remember { mutableStateOf("") }
    val avatarUrl = remember(profile) { mutableStateOf(profile?.avatar_url) }
        ...
        if (uri != null) {
            scope.launch {
                ...
                if (file != null) {
                    ...
                    val response = runCatching { ApiClient.service.uploadAvatar(part) }.getOrNull()
                    file.delete()
    ...
    LaunchedEffect(reviewPage.value) {
        val response = runCatching { ApiClient.service.userReviews(reviewPage.value) }.getOrNull()
        if (response?.success == true) {
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column {
                            ...
                            Text(profile?.email ?: "example@mail.com", style = MaterialTheme.typography.bodySmall)
                        }
                    }
        ...
        items(userReviews.value) { review ->
            Card(
                ...
                Column(modifier = Modifier.padding(12.dp)) {
                    Text(review.place_name ?: "İşletme", style = MaterialTheme.typography.labelSmall, color = Color.Gray)
                    Text(review.text ?: "", style = MaterialTheme.typography.bodyMedium)
                    Text(review.created_at ?: "", style = MaterialTheme.typography.labelSmall, color = Color.Gray)
                }
            }
        }
