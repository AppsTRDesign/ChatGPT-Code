...
private fun ProfileLoggedIn(
    ...
    Column(
        ...
        when (tabIndex.value) {
            ...
            2 -> {
                val approvedReviews = userReviews.value.filter { it.status == "approved" }
                val pendingReviews = userReviews.value.filter { it.status != "approved" }
                val activeReviews = if (reviewTabIndex.value == 0) approvedReviews else pendingReviews
                LazyColumn(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.spacedBy(12.dp)
                ) {
                    item {
                        ...
                        Spacer(modifier = Modifier.height(8.dp))
                        TabRow(selectedTabIndex = reviewTabIndex.value) {
                            Tab(
                ...
                val answeredReviews = userReviews.value.filter { it.status == "approved" }
                val unansweredReviews = userReviews.value.filter { it.status != "approved" }
                val activeReviews = if (ownerReviewTabIndex.value == 0) answeredReviews else unansweredReviews
                LazyColumn(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.spacedBy(12.dp)
                ) {
                    item {
                        ...
                        Spacer(modifier = Modifier.height(8.dp))
                        TabRow(selectedTabIndex = ownerReviewTabIndex.value) {
                            Tab(
