...
fun SearchScreen(viewModel: AppStateViewModel, navController: NavController) {
    ...
    LaunchedEffect(searchLoading, searchTotal, searchResults.size) {
        if (searchTriggered.value && !searchLoading) {
            val total = if (searchTotal > 0) searchTotal else searchResults.size
            Toast.makeText(
                context,
                "Arama tamamlandı: $total sonuç",
                Toast.LENGTH_SHORT
            ).show()
