...
if ($userId > 0) {
    ...
    $place['is_favorite'] = (bool)$favStmt->fetchColumn();
}
...
if (is_array($googleReviews)) {
    foreach ($googleReviews as &$gr) {
        ...
        $likeKey = 'google:' . $googleRef;
        $gr['reply'] = $replyMap[$replyKey] ?? null;
        $gr['likes_count'] = $likeCounts[$likeKey] ?? 0;
        $gr['liked_by_user'] = !empty($likedByUser[$likeKey]);
    }
    unset($gr);
...
foreach ($reviewStmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    $reviews[] = [
        ...
        'likes_count' => $likeCounts['user:' . $row['id']] ?? 0,
        'liked_by_user' => !empty($likedByUser['user:' . $row['id']]),
        'reply' => $reply
    ];
}
