...
switch ($action) {
    ...
    case 'category-image-delete':
        ...
        db()->prepare('UPDATE categories SET image = NULL WHERE id = :id')->execute(['id' => $categoryId]);
        echo json_encode(['success' => true, 'message' => 'Kategori resmi silindi.']);
        break;
    case 'product-image-delete':
        ...
        db()->prepare('UPDATE products SET main_image = NULL WHERE id = :id')->execute(['id' => $productId]);
        echo json_encode(['success' => true, 'message' => 'Ürün görseli silindi.']);
        break;
    case 'product-gallery-delete':
        ...
        if (!$imageId) {
            http_response_code(422);
            echo json_encode(['success' => false, 'message' => 'Galeri resmi bulunamadı.']);
            break;
        }
        ...
        echo json_encode(['success' => true, 'message' => 'Galeri resmi silindi.']);
        break;
    case 'setting-image-delete':
        if (!is_admin()) {
            http_response_code(401);
            echo json_encode(['success' => false, 'message' => 'Yetkisiz.']);
            break;
        ...
        if (!in_array($key, ['logo', 'favicon'], true)) {
            http_response_code(422);
            echo json_encode(['success' => false, 'message' => 'Geçersiz ayar.']);
            break;
        }
        $path = settings($key);
    ...
    case 'shipper':
        ...
        if ($shipperId) {
            ...
            db()->prepare('UPDATE shippers SET name = :name, address = :address, website = :website, query_url = :query_url, api_url = :api_url, logo = COALESCE(:logo, logo) WHERE id = :id')
                ->execute([
                    'id' => $shipperId,
                    'name' => $name,
                    'address' => trim($_POST['address'] ?? ''),
                    'website' => trim($_POST['website'] ?? ''),
                    'query_url' => trim($_POST['query_url'] ?? ''),
                    'api_url' => trim($_POST['api_url'] ?? ''),
                    'logo' => $logo,
                ]);
        } else {
            db()->prepare('INSERT INTO shippers (name, address, website, query_url, api_url, logo, created_at) VALUES (:name, :address, :website, :query_url, :api_url, :logo, :created_at)')
                ->execute([
                    'name' => $name,
                    'address' => trim($_POST['address'] ?? ''),
                    'website' => trim($_POST['website'] ?? ''),
