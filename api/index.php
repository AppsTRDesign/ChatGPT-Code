...
function register(PDO $pdo, array $config): void
{
    ...
    $allowMulti = isset($data['allow_multi_account']) ? (int)$data['allow_multi_account'] : 0;
    ...
    if (!$deviceId) {
        Response::error('device_id zorunlu');
        return;
    }
    ...
    $deviceOwner = null;
    $deviceLookup = $pdo->prepare('SELECT allow_multi_account, max_multi_accounts FROM users WHERE device_fingerprint = ? LIMIT 1');
    $deviceLookup->execute([$deviceId]);
    $deviceOwner = $deviceLookup->fetch();
    if ($deviceOwner) {
        $countStmt = $pdo->prepare('SELECT COUNT(*) FROM users WHERE device_fingerprint = ?');
        $countStmt->execute([$deviceId]);
        $deviceCount = (int)$countStmt->fetchColumn();
        $maxAllowed = (int)($deviceOwner['max_multi_accounts'] ?? 5);
        if ((int)$deviceOwner['allow_multi_account'] === 0) {
            Response::error('Bu cihaz için kayıt sınırı dolu', 429);
            return;
        }
        if ($deviceCount >= $maxAllowed) {
            Response::error('Bu cihaz için maksimum hesap sayısına ulaşıldı', 429);
            return;
        }
    }

    $hash = password_hash($password, PASSWORD_BCRYPT);
    $pdo->prepare('INSERT INTO users (email, password_hash, display_name, points, device_fingerprint, allow_multi_account, max_multi_accounts) VALUES (?, ?, ?, ?, ?, ?, ?)')
        ->execute([
            $email,
            $hash,
            $name,
...
function login(PDO $pdo, array $config): void
{
    ...
    if ($deviceId) {
        if (!empty($user['device_fingerprint']) && $user['device_fingerprint'] !== $deviceId && (int)$user['allow_multi_account'] === 0) {
            Response::error('Bu cihaz için giriş izni yok', 403);
            return;
        }
        if (empty($user['device_fingerprint'])) {
            $pdo->prepare('UPDATE users SET device_fingerprint = ? WHERE id = ?')->execute([$deviceId, $user['id']]);
    ...
    try {
        ...
        foreach ($sites as &$s) {
            ...
            if (is_array($raw)) {
                ...
                $s['media_actions'] = json_decode($raw, true) ?: [];
            } else {
                $s['media_actions'] = [];
            }
        }
...
function start_surf(PDO $pdo, array $user, array $config): void
{
    ...
    $visitLimit = (int)($user['max_daily_site_visits'] ?? 0);
        ...
        try {
            ...
            $pdo->commit();
            $rawActions = $site['media_actions'] ?? '';
            if (is_array($rawActions)) {
                $site['media_actions'] = $rawActions;
            } elseif (is_string($rawActions) && $rawActions !== '') {
                $site['media_actions'] = json_decode($rawActions, true) ?: [];
            } else {
                $site['media_actions'] = [];
            }
...
function complete_surf(PDO $pdo, array $user, array $config): void
{
    ...
    $pdo->beginTransaction();
    $stmt = $pdo->prepare('SELECT ss.*, s.user_id AS owner_id, s.dwell_seconds, s.google_pages, s.youtube_pages FROM surf_sessions ss JOIN sites s ON s.id = ss.site_id WHERE ss.id = ? AND ss.surfer_id = ? FOR UPDATE');
    ...
    if ($ownerId !== (int)$user['id'] && $baseReward > 0) {
        ...
        $available = max(0, (int)($owner['points'] ?? 0));
        $spend = min($baseReward, $available);
        $rewardCap = clamp_reward($pdo, $user, $baseReward, $config);
        $reward = min($spend, $rewardCap);
        if ($reward > 0) {
            $pdo->prepare('UPDATE users SET points = points - ? WHERE id = ?')->execute([$reward, $ownerId]);
