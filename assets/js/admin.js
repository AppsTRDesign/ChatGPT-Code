...
const bindMediaDeleteButtons = (selector, action, payloadKey) => {
  document.querySelectorAll(selector).forEach((button) => {
    button.addEventListener('click', async () => {
      const value = button.dataset[payloadKey] || '';
      if (!value) return;
      const formData = new FormData();
      formData.append('csrf_token', document.querySelector('input[name="csrf_token"]')?.value || '');
      formData.append(payloadKey === 'deleteSettingImage' ? 'key' : 'id', value);
      const response = await fetch(`/api/handler.php?action=${action}`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (window.toastr) {
        if (response.ok) {
          toastr.success(data.message || 'Silindi.');
          button.closest('[data-remove-on-delete]')?.remove();
        } else {
          toastr.error(data.message || 'Silinemedi.');
        }
      }
...
bindMediaDeleteButtons('[data-delete-category-image]', 'category-image-delete', 'deleteCategoryImage');
