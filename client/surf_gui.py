...
class SurfApp(QtWidgets.QMainWindow):
    ...
    def _refresh_ad_banner(self):
        default_html = (
            "<div style='margin:0;padding:0;text-align:center;background:#0f172a;'>"
            "<a href=\"https://noasoft.org\" target=\"_blank\">"
            "<img src=\"https://picsum.photos/1200/90\" style=\"width:100%;max-width:1200px;\">"
            "</a></div>"
        )
        ad_html = ""
        ad_enabled = True
        ...
        if isinstance(self.task_points, dict):
            ad_html = self.task_points.get("ad_banner_html", "") or ""
        ...
        ad_html = ad_html if ad_html.strip() else default_html
        if getattr(self, "ad_view", None) is not None:
            ...
            full_html = (
                ...
                "<style>body{margin:0;padding:0;background:#0f172a;text-align:center;}img{max-width:1200px;width:100%;}</style>"
                "</head><body>"
                f"{ad_html}"
                "</body></html>"
            )
