...
$regionStmt = $pdo->prepare('INSERT INTO regions(id,country_id,country_code,country_name,name,slug,lat,lng,polygon_json,population,owner_region_id,region_type,government_type,color,flag_url,neighbors_json,created_at) VALUES(:id,:country_id,:country_code,:country_name,:name,:slug,:lat,:lng,:polygon_json,:population,NULL,:region_type,:government_type,:color,:flag_url,JSON_ARRAY(),NOW())');
...
$regions = $pdo->query('SELECT id,country_id,owner_region_id,region_type FROM regions')->fetchAll();
