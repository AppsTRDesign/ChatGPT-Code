...
switch ($route) {
    ...
    case 'son-yorumlar':
        $page = (int)($_GET['s'] ?? 1);
        [$offset, $limit] = paginate($page, RECENT_REVIEW_LIMIT);
        $total = (int)$pdo->query('SELECT COUNT(*) FROM user_reviews')->fetchColumn();
        $totalPages = (int)ceil(max(1, $total) / $limit);
        $stmt = $pdo->prepare('SELECT ur.place_id, ur.author_name, ur.email, ur.rating, ur.review_text, ur.text_extra, ur.created_at,
                p.name, p.business_image, p.business_type, p.category_slug, p.city_name, p.formatted_address
            FROM user_reviews ur
            INNER JOIN places p ON p.id = ur.place_id
        ...
        foreach ($rows as &$row) {
            $row['text_extra'] = decode_json($row['text_extra'] ?? '');
            $row['slug'] = build_place_slug(['id' => $row['place_id'], 'name' => $row['name'] ?? '']);
        }
        render(__DIR__ . '/templates/recent_reviews.php', [
            'meta' => render_meta(['title' => 'Son Yorumlar', 'description' => 'En yeni kullanıcı yorumları', 'url' => BASE_URL . '/son-yorumlar']),
            'reviews' => $rows,
            'page' => $page,
            'totalPages' => $totalPages,
            'totalReviews' => $total,
