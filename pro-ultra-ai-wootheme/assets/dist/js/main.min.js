...
function renderProductCards(e){const t=document.createElement("div");return t.className="pro-ultra-ai-chat-products",e.forEach((e=>{const r=document.createElement("article");if(r.className="pro-ultra-ai-chat-product",e.thumb){const t=document.createElement("img");t.src=e.thumb,t.alt=e.title,r.appendChild(t)}const a=document.createElement("h5");if(a.textContent=e.title,r.appendChild(a),e.price){const t=document.createElement("p");t.className="price",t.textContent=e.price,r.appendChild(t)}const n=document.createElement("a");n.href=e.url,n.textContent=proUltraAIChat&&proUltraAIChat.viewLabel?proUltraAIChat.viewLabel:"Görüntüle",n.className="button",r.appendChild(n),t.appendChild(r)})),t}function renderMessage(e){if(!chatBody)return;const t=document.createElement("div");t.className="pro-ultra-ai-chat-message "+("ai"===e.role?"is-ai":"is-user");const r=document.createElement("span");if(r.className="pro-ultra-ai-chat-message__bubble",r.textContent=e.text,t.appendChild(r),e.link){const r=document.createElement("a");r.href=e.link,r.className="pro-ultra-ai-chat-link",r.textContent=r.href,t.appendChild(r)}e.products&&e.products.length&&t.appendChild(renderProductCards(e.products)),chatBody.appendChild(t),chatBody.scrollTop=chatBody.scrollHeight}
...
.then(res=>res.json())
.then(res=>{const t=res.data||{},e=res.success&&t?t.reply:t.message||proUltraAIChat.errorText,a={role:"ai",text:e,products:t.products||[],link:t.comparison_link};chatHistory.push(a),renderMessage(a),saveHistory();})
.catch(()=>{
