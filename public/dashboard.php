...
async function loadRegions(){
  ...
  const rows=REGION_ROWS.filter(x=>Number(x.owner_region_id)===selected).map(x=>`<div class='glass' style='padding:10px;margin:6px 0'><b>${x.name}</b> (${x.region_type})<br>Total: ${x.total_score} • Airport ${x.airport_level} • Army ${x.army_level} • Hospital ${x.hospital_level} • Education ${x.education_level} • School ${x.school_level} • Port ${x.port_level}</div>`).join('');
...
async function showRegionDetail(id){
  ...
  detailPanel.innerHTML=`<div class='glass' style='padding:10px'><b>${r.name}</b><br>Total: ${r.total_score}<br>Airport ${r.airport_level} • Army ${r.army_level} • Hospital ${r.hospital_level} • Education ${r.education_level} • School ${r.school_level} • Port ${r.port_level}<br><button class='primary-btn' id='travelRegionBtn'>Travel</button></div>`;
...
async function showCountryDetail(id){
  ...
  const regs=(c.regions||[]).map(x=>`${x.name} (${Number(x.airport_count)+Number(x.army_count)+Number(x.hospital_count)+Number(x.education_count)+Number(x.school_count)+Number(x.port_count)})`).join(', ');
