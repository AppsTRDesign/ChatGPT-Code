...
TS="$(date +%s)"
USERNAME="action_${TS}"
EMAIL="action_${TS}@mail.com"

register_payload=$(cat <<JSON
{"username":"$USERNAME","email":"$EMAIL","password":"Secret123"}
JSON
)
reg=$(curl -s -X POST "$BASE_URL/api/auth/register" -H 'Content-Type: application/json' -d "$register_payload")
token=$(php -r '$j=json_decode($argv[1],true); echo $j["token"] ?? "";' "$reg")
...
valid=$(curl -s -X POST "$BASE_URL/api/region/action" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d "$valid_payload")
...
same_payload=$(cat <<JSON
{"region_id":$dest,"action":"travel"}
JSON
)
same=$(curl -s -X POST "$BASE_URL/api/region/action" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d "$same_payload")
...
bad=$(curl -s -X POST "$BASE_URL/api/region/action" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d '{"region_id":999999,"action":"travel"}')
